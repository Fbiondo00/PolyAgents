...
Arc è la L1 EVM di Circle — USDC è il gas token nativo, non ETH. Deploy esattamente come Ethereum con Foundry/Hardhat, ma ogni fee è in dollari stabili. Per VaultPilot è perfetto: vault USDC, fee USDC, tutto denominato in dollari.bim+1
...
text
// SPDX-License-Identifier: MIT pragma solidity ^0.8.24;   import "forge-std/Script.sol"; import "../src/VaultPilotMarket.sol";   contract DeployScript is Script {  // Indirizzo USDC su Arc Testnet — verifica su docs.arc.network address constant ARC\_TESTNET\_USDC = 0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238;   function run() external { uint256 deployerKey = vm.envUint("ARC\_PRIVATE\_KEY"); vm.startBroadcast(deployerKey);   VaultPilotMarket market = new VaultPilotMarket(ARC\_TESTNET\_USDC);   console.log("VaultPilotMarket deployed at:", address(market)); console.log("Explorer: https://explorer.testnet.arc.network/address/", address(market));   vm.stopBroadcast(); } }
Deploy:
bash
cd contracts   \# Compila prima forge build   \# Deploy forge script script/Deploy.s.sol \  --rpc-url $ARC\_TESTNET\_RPC_URL  \  --private-key $ARC\_PRIVATE\_KEY  \  --broadcast \ --verify   \# Output atteso: \# VaultPilotMarket deployed at: 0xABCD... \# Explorer: https://explorer.testnet.arc.network/address/0xABCD...
Salva l'indirizzo nel .env:
text
VAULTPILOT\_CONTRACT\_ADDRESS=0xABCD...
...
});  await publicClient.waitForTransactionReceipt({ hash: approveTx });    // 2. Piazza la scommessa  const txHash =  await walletClient.writeContract({  address: contractAddress,  abi:  VAULTPILOT_ABI,  functionName:  "placeBet",  args:
});    await publicClient.waitForTransactionReceipt({ hash: txHash });  console.log(💰 Bet placed on market #${opts.marketId} → ${txHash});  return txHash; }   // ── Risolvi mercato ── export  async  function  resolveMarket(  marketId: bigint,  outcomeYes:  boolean ):  Promise<string>  {  const outcome = outcomeYes ?  1  :  2;  // 1=YES, 2=NO    const txHash =  await walletClient.writeContract({  address: contractAddress,  abi:  VAULTPILOT_ABI,  functionName:  "resolveMarket",  args:
});    await publicClient.waitForTransactionReceipt({ hash: txHash });  return txHash; }   // ── Read-only: stato mercato ── export  async  function  getMarketData(marketId: bigint)  {  const market =  await publicClient.readContract({  address: contractAddress,  abi:  VAULTPILOT_ABI,  functionName:  "getMarket",  args:
...
text
┌─────────────────────────────────────────────────────────┐ │                    VaultPilot                           │ │                                                         │ │  ┌──────────┐    ┌──────────────┐    ┌──────────────┐  │ │  │  React   │───▶│  Express API │───▶│  AI Agent    │  │ │  │ Frontend │◀───│   Backend    │◀───│  (LLM Loop)  │  │ │  └──────────┘    └──────┬───────┘    └──────┬───────┘  │ │                         │                   │           │ └─────────────────────────┼───────────────────┼───────────┘  │                   │ ┌────────────┼───────────────────┼─────────┐ │            ▼                   ▼         │ │   ┌────────────────┐  ┌──────────────┐   │ │   │  Arc Testnet   │  │   Hedera     │   │ │   │  (EVM L1)      │  │   Testnet    │   │ │   │                │  │              │   │ │   │ VaultPilotMarkt│  │ HTS Token    │   │ │   │ \- createMarket │  │ HCS Audit    │   │ │   │ \- placeBet     │  │ HCS-14 UAID  │   │ │   │ \- resolveMarket│  │ Scheduled TX │   │ │   │ USDC gas token │  │ Mirror Node  │   │ │   └────────────────┘  └──────────────┘   │ └────────────────────────────────────────────┘ │ ┌────────────▼───────────┐ │  ENS (Sepolia)          │ │  vault.vaultpilot.eth   │ │  text: policy.commitment│ │  text: agent.lastTrade  │ └────────────────────────┘
